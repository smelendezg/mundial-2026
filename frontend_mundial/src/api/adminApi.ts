...
export async function adminCreateMatch(payload: {
  ...
  if (!USE_MOCK) {
    ...
    await createSystemEvent({
      ...
      data: {
        ...
        startTimeISO: created.startTimeISO,
  ...
  if (assign) {
    for (const p of poolsMock) {
      if (!p.matchIds.includes(match.id)) {
        p.matchIds.push(match.id);
      }
    }
  }
...
export async function adminSetMatchStatus(
  ...
  if (!USE_MOCK) {
    const match = await http.patch<Match>(`/admin/matches/${matchId}/status`, {
      status,
    });
  ...
  if (status === "FINISHED" && match.score) {
...
export async function adminPublishResult(
  matchId: string,
  home: number,
  away: number
): Promise<Match> {
  if (!USE_MOCK) {
    const match = await http.post<Match>(`/admin/matches/${matchId}/result`, {
      homeScore: home,
      awayScore: away,
    });
